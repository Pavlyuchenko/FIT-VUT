...
def udp_auth_nok_ok(tester):
    ...
    ), "Incoming message does not match expected CONFIRM message 2."
...
@testcase
def udp_mes_snd_rec(tester):
    auth_and_reply(tester)
    
    tester.execute("hello there!")
    
    tester.send_message(msg(1, "vitej", "svete"))
    
    sleep(0.2)
    stderr = tester.get_stdout()
    ...
    assert ("vitej: svete" in stderr), "wrong" + stderr
    assert ("MSG FROM c IS hello there!" in tMessage), "wrong" + tMessage
